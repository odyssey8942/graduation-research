...
for i in range(1):
    ...
    #model_path = "G:/cuda/test1/new_slip_model/10_128_4_5_10_4950_" + str(i+1) + "0_use_train_slip_model.npz"
    model_path = "G:/cuda/test1/new_slip_model/10_128_4_5_10_4950_30_use_train_slip_model.npz"
    ...
    for row in predict:
        ...
        #"""
    """
    #print(f1_score(isslip_train,isslip_predict,average="macro"))
    if(f1_score(isslip_train,isslip_predict,average="macro") > maxaccuracy):
        maxaccuracy = f1_score(isslip_train,isslip_predict,average="macro")
        maxnum = (i+1)*10
    #"""
    N = len(test_t)
    #"""
    #plt.plot(range(N), test_fl, color="red", label="train_front_left_slipratio", linestyle="-")
    #plt.plot(range(N), test_fr, color="red", label="train_front_right_slipratio", linestyle="-")
    #plt.plot(range(N), test_rl, color="red", label="train_rear_left_slipratio", linestyle="-")
    #plt.plot(range(N), test_rr, color="red", label="train_rear_right_slipratio", linestyle="-")

    #plt.plot(range(N), predict_fl, color="blue", label="predict_front_left_slipratio", linestyle="--")
    #plt.plot(range(N), predict_fr, color="blue", label="predict_front_right_slipratio", linestyle="--")
    #plt.plot(range(N), predict_rl, color="blue", label="predict_rear_left_slipratio", linestyle="--")
    #plt.plot(range(N), predict_rr, color="blue", label="predict_rear_right_slipratio", linestyle="--")
    plt.plot(range(N), isslip_train, color="red", label="slip_train")
